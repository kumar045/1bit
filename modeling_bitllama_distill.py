...
def ternary_quantize_groupwise(
    ...
    Maps to {-1, 0, +1} and rescales using per-group absmean scale.
    """
    orig_shape = w.shape
    last_dim = w.shape[-1]
...
def quantize_with_ste(w_real: torch.Tensor, group_size: int) -> torch.Tensor:
    """
    Forward uses quantized weights, backward updates latent real weights.
    """
...
class BitLlamaModel(BitLlamaPreTrainedModel):
    ...
    def forward(
        ...
        if output_hidden_states:
            # replace final layer output with normalized final output
...
# =========================================================
# Causal LM head with loss decomposition
# =========================================================
...
class BitLlamaForCausalLM(BitLlamaPreTrainedModel):
    ...
    @staticmethod
    def _match_teacher_student_hidden(
        student_hiddens: Tuple[torch.Tensor, ...],
        teacher_hiddens: Tuple[torch.Tensor, ...],
    ) -> List[Tuple[torch.Tensor, torch.Tensor]]:
        """
        Map student hidden states to teacher hidden states by evenly spacing indices.
        Includes embedding state + layer outputs.
        """
        ...
        if s_len == t_len:
            return list(zip(student_hiddens, teacher_hiddens))

        pairs = []
        for i in range(s_len):
            t_idx = round(i * (t_len - 1) / max(s_len - 1, 1))
            pairs.append((student_hiddens[i], teacher_hiddens[t_idx]))
        return pairs
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            if teacher_hidden_states is not None and all_hidden_states is not None:
                pairs = self._match_teacher_student_hidden(all_hidden_states, teacher_hidden_states)

                hidden_terms = []
                for s_h, t_h in pairs:
                    # simple normalized MSE to reduce scale mismatch pain
                    s_norm = F.normalize(s_h.float(), dim=-1)
                    t_norm = F.normalize(t_h.float(), dim=-1)
                    hidden_terms.append(F.mse_loss(s_norm, t_norm))
