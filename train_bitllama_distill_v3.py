...
from modeling_bitllama_distill_v3 import BitLlamaConfig, BitLlamaForCausalLM
...
class ScriptArgs:
    ...
    output_dir: str = field(default="./bitllama_out_v3")
...
class DistillMetricsCallback(TrainerCallback):
    def on_log(self, args, state, control, logs=None, **kwargs):
        if not logs:
            return
        keep = ["loss", "ce_loss", "kl_loss", "hidden_loss", "eval_loss"]
        filtered = {k: v for k, v in logs.items() if k in keep}
        if filtered:
...
class DistillTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        extra_logs = {}
        for key in ["ce_loss", "kl_loss", "hidden_loss"]:
            value = outputs.get(key, None)
            if value is not None:
                extra_logs[key] = value.detach().float().item()
        if extra_logs:
            self.log(extra_logs)
